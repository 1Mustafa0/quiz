...
import { ChevronLeft, ChevronRight, Clock, AlertCircle, CheckCircle2, XCircle, Send, LogOut } from 'lucide-react';
...
const QuizPlayer: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-20 relative">
      {/* Header & Timer */}
      <div className="flex items-center justify-between bg-white p-6 rounded-2xl border border-gray-100 shadow-sm sticky top-20 z-40">
        ...
        <div className={`flex items-center px-4 py-2 rounded-xl font-mono font-bold text-lg ${timeLeft < 60 ? 'bg-red-50 text-red-600 animate-pulse' : 'bg-indigo-50 text-indigo-600'}`}>
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit={{ opacity: 0, x: -20 }}
          className="bg-white p-8 rounded-3xl border border-gray-100 shadow-lg space-y-8"
        >
          <div className="space-y-4">
            <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-indigo-50 text-indigo-600 uppercase tracking-wider">
                ...
                {currentQuestion.options.map((opt, i) => (
                  ...
                  >
                    ...
                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors ${
                      ...
                      {userAnswers[currentQuestionIndex] === opt && <div className="w-2 h-2 bg-white rounded-full" />}
                    </div>
                  </button>
                ))}
              </div>
            ...
            {currentQuestion.type === 'short-answer' && (
              <div className="space-y-2">
                <input
                  ...
                  className="w-full p-6 bg-gray-50 border-2 border-gray-100 rounded-2xl text-lg font-medium focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
                />
                <p className="text-xs text-gray-400 italic px-2 text-right">الأسئلة ذات الإجابة القصيرة لا تتأثر بحالة الأحرف (كبيرة/صغيرة).</p>
              </div>
            )}
          </div>
        </motion.div>
      ...
      <div className="flex items-center justify-between">
        <button
          ...
          className="inline-flex items-center px-6 py-3 bg-white border border-gray-200 text-gray-700 rounded-xl font-semibold hover:bg-gray-50 transition-all disabled:opacity-50"
        >
        ...
        {currentQuestionIndex === quiz.questions.length - 1 ? (
          <button
            ...
            className="inline-flex items-center px-8 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 shadow-lg transition-all transform hover:scale-105"
          >
            إنهاء الكويز
          ...
          <button
            ...
            className="inline-flex items-center px-6 py-3 bg-white border border-gray-200 text-gray-700 rounded-xl font-semibold hover:bg-gray-50 transition-all"
          >
            التالي
